...
#define KDA_FFI_CHECK(cond, msg) \
  do { if (!(cond)) { throw std::runtime_error(std::string("kda_flash: ") + (msg)); } } while (0)
...
inline void CheckCuda(cudaError_t status, const char* operation) {
  if (status != cudaSuccess) {
    throw std::runtime_error(std::string("kda_flash: ") + operation + " failed: " +
...
inline CUtensorMap EncodeQkTma(const void* base, int64_t numel, int64_t d2, int64_t d1,
  ...
  const CUresult result =
      cuTensorMapEncodeTiled(&tensor_map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 4,
                             const_cast<void*>(base), global_dim, global_strides, box_dim,
                             elem_strides, CU_TENSOR_MAP_INTERLEAVE_NONE,
                             CU_TENSOR_MAP_SWIZZLE_128B, CU_TENSOR_MAP_L2_PROMOTION_NONE,
                             CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
  KDA_FFI_CHECK(result == CUDA_SUCCESS,
...
inline TmaPointers EncodeTmaPointersAll(const void* q, const void* k, const void* v, const void* g,
  ...
  if (prepare_descriptors != 0) {
    ...
    PublishTensorMaps<<<1, 128, 0, stream>>>(
        reinterpret_cast<uint64_t*>(descriptor_storage_ptr), words);
  ...
  return {bytes + 0 * stride, bytes + 1 * stride, bytes + 2 * stride,
          bytes + 3 * stride, bytes + 4 * stride, bytes + 5 * stride, bytes + 6 * stride};
}
