...
__global__ void __launch_bounds__(32) kernel_flashkda_persistent_plan(
    const long long* __restrict__ cu_seqlens, const int* __restrict__ seq_order,
    int num_seqs, int num_heads, int sm_count, int* __restrict__ task_ids,
    int* __restrict__ task_offsets, int* __restrict__ choice_scratch) {
...
void RunPersistentM128(const void* q_ptr, const void* k_ptr, const void* v_ptr,
  ...
  kda_flash::CheckCuda(cudaFuncSetAttribute(kernel_flashkda_bf16_persistent_m128,
  ...
  kernel_flashkda_persistent_plan<<<1, 32, 0, stream>>>(  // NOLINT: single-block LPT planner
      ...
      static_cast<int>(num_heads), static_cast<int>(workers),
      reinterpret_cast<int*>(task_ids_ptr), reinterpret_cast<int*>(task_offsets_ptr),
      reinterpret_cast<int*>(choice_scratch_ptr));
