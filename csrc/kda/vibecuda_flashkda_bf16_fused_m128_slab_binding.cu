...
__global__ __launch_bounds__(256) void kernel_flashkda_split_carry_copy_m128_slab(
  ...
  const float4* src =
      reinterpret_cast<const float4*>(split_state + task * 2 * 16384);
...
static void LaunchM128(const TmaPointers& tma, const void* q_ptr, const void* k_ptr,
                       ...
                       int64_t num_seqs, int64_t num_heads, int64_t use_initial_state,
                       int64_t store_final_state, double scale, double lower_bound,
                       int64_t split_num_parts, void* split_state_ptr, void* split_gamma_ptr,
                       int64_t fixup_mode, int64_t grid_y, cudaStream_t stream,
                       int* progress_flags, void* map_state_bf16_ptr, int64_t ft_slab) {
...
void RunM128(const void* q_ptr, const void* k_ptr, const void* v_ptr, const void* g_ptr,
             ...
             int64_t store_final_state, double scale, double lower_bound,
             int64_t beta_tma_rows, int64_t beta_tma_dim1, int64_t ft_slab, int64_t cuda_stream) {
  constexpr int32_t kSmemBytes = SMEM_TOTAL;
  kda_flash::CheckCuda(cudaFuncSetAttribute(kernel_flashkda_bf16_fused_m128,
...
void RunM128SplitFull(const void* q_ptr, const void* k_ptr, const void* v_ptr, const void* g_ptr,
                      ...
                      void* lookback_flags_ptr, void* map_state_bf16_ptr,
  ...
  kda_flash::CheckCuda(cudaFuncSetAttribute(kernel_flashkda_bf16_fused_m128,
                                            ...
                                            kSmemBytes),
                       "cudaFuncSetAttribute(kernel_flashkda_bf16_fused_m128)");
  kda_flash::CheckCuda(cudaFuncSetAttribute(kernel_flashkda_split_scan_bf16_m128,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize,
  ...
  LaunchM128(tma, q_ptr, k_ptr, v_ptr, g_ptr, beta_ptr, A_log_ptr, dt_bias_ptr, cu_seqlens_ptr,
             seq_order_ptr, initial_state_ptr, out_ptr, final_state_ptr, num_seqs, num_heads,
             use_initial_state, /*store_final_state=*/0, scale, lower_bound,
             num_parts, split_state_ptr, /*split_gamma_ptr=*/nullptr, /*fixup_mode=*/0,
  ...
  LaunchM128(tma, q_ptr, k_ptr, v_ptr, g_ptr, beta_ptr, A_log_ptr, dt_bias_ptr, cu_seqlens_ptr,
             seq_order_ptr, initial_state_ptr, out_ptr, final_state_ptr, num_seqs, num_heads,
             /*use_initial_state=*/0, /*store_final_state=*/0, scale, lower_bound,
             num_parts, map_state_ptr, /*split_gamma_ptr=*/nullptr, /*fixup_mode=*/2,
             /*grid_y=*/num_parts, stream, /*progress_flags=*/nullptr,
             map_state_bf16_ptr, ft_slab);
  }
  ...
  if (kP2Spec) {
    kernel_flashkda_split_carry_copy_m128_slab<<<
        dim3(static_cast<uint32_t>(num_tasks)), 256, 0, stream>>>(
        reinterpret_cast<const float*>(split_state_ptr),
        reinterpret_cast<float*>(carry_ptr),
        reinterpret_cast<const int*>(seq_order_ptr), static_cast<int>(num_heads));
    CheckCuda(cudaGetLastError(),
              "kernel_flashkda_split_carry_copy_m128_slab launch");
  } else {
    ...
    kernel_flashkda_split_scan_bf16_m128<<<dim3(static_cast<uint32_t>(num_tasks), 32),
                                           128, 65552, stream>>>(
