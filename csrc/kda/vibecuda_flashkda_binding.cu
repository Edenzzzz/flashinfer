...
void RunM128SplitFull(const void* q_ptr, const void* k_ptr, const void* v_ptr, const void* g_ptr,
                      ...
                      void* descriptor_storage_ptr, int64_t token_count, int64_t num_seqs,
                      int64_t prepare_descriptors, int64_t num_heads, int64_t use_initial_state,
                      double scale, double lower_bound, int64_t beta_tma_rows,
                      int64_t beta_tma_dim1, int64_t num_parts, void* split_state_ptr,
                      void* map_state_ptr, void* carry_ptr, void* split_out_ptr,
                      void* lookback_flags_ptr, void* map_state_bf16_ptr, int64_t ft_slab,
                      int64_t cuda_stream);
...
void RunPersistentM128(const void* q_ptr, const void* k_ptr, const void* v_ptr,
                       const void* g_ptr, const void* beta_ptr, const void* beta_tma_ptr,
                       const void* A_log_ptr, const void* dt_bias_ptr,
                       const void* cu_seqlens_ptr, const void* seq_order_ptr,
                       void* task_ids_ptr, void* task_offsets_ptr, void* choice_scratch_ptr,
                       const void* initial_state_ptr, void* out_ptr, void* final_state_ptr,
                       void* descriptor_storage_ptr, int64_t token_count, int64_t num_seqs,
                       int64_t prepare_descriptors, int64_t num_heads, int64_t use_initial_state,
                       int64_t store_final_state, double scale, double lower_bound,
                       int64_t beta_tma_rows, int64_t beta_tma_dim1, int64_t sm_count,
                       int64_t cuda_stream);
}  // namespace kda_flash
...
// rules.
int64_t CheckVibeCUDAPrefillInputs(const TensorView& q, const TensorView& k,
  ...
  TVM_FFI_ICHECK(std::isfinite(lower_bound) && lower_bound < 0.0 &&
      ...
      << "lower_bound must be finite, negative, and representable as float32, got "
      << lower_bound;
  ...
  TVM_FFI_ICHECK(seq_order.numel() == num_seqs)
      << "seq_order must have one entry per sequence";

  if (use_initial_state != 0) {
    TVM_FFI_ICHECK(initial_state.numel() == num_seqs * num_heads * flash_kda::kHeadDim *
                                               flash_kda::kHeadDim)
...
__global__ void VibeCUDAStableDescLengthSort(const int64_t* __restrict__ cu_seqlens,
                                             int num_seqs, int* __restrict__ order) {
...
void SortSeqsInto(TensorView cu_seqlens, TensorView order_out, int64_t cuda_stream) {
  ...
  const int64_t num_seqs = cu_seqlens.numel() - 1;
  TVM_FFI_ICHECK(num_seqs > 0) << "cu_seqlens must describe at least one sequence";
  TVM_FFI_ICHECK(order_out.numel() == num_seqs)
      << "order_out must have one entry per sequence";
...
void RunM128(TensorView q, TensorView k, TensorView v, TensorView g, TensorView beta,
  ...
  if (slab != 0) {
    kda_flash_slab::RunM128(q.data_ptr(), k.data_ptr(), v.data_ptr(), g.data_ptr(),
                            beta.data_ptr(), beta_tma.data_ptr(), A_log.data_ptr(),
                            dt_bias.data_ptr(), cu_seqlens.data_ptr(), seq_order.data_ptr(),
                            initial_state.data_ptr(), out.data_ptr(), final_state.data_ptr(),
                            descriptor_storage.data_ptr(), token_count, num_seqs,
                            prepare_descriptors, num_heads, use_initial_state, store_final_state,
                            scale, lower_bound, beta_tma_rows, beta_tma_dim1, slab, cuda_stream);
  } else {
...
void RunPersistentM128(TensorView q, TensorView k, TensorView v, TensorView g, TensorView beta,
                       ...
                       TensorView task_offsets, TensorView choice_scratch,
                       TensorView initial_state, TensorView out, TensorView final_state,
                       TensorView descriptor_storage, int64_t prepare_descriptors,
                       int64_t num_heads, int64_t use_initial_state, double scale,
                       double lower_bound, int64_t sm_count, int64_t cuda_stream) {
  TVM_FFI_ICHECK(cuda_stream >= 0) << "cuda_stream must be a non-negative stream handle";
  TVM_FFI_ICHECK(q.device().device_type == kDLCUDA) << "q must be a CUDA tensor";
  const int32_t device_id = q.device().device_id;
  ffi::CUDADeviceGuard device_guard(device_id);
...
void RunM128Split(TensorView q, TensorView k, TensorView v, TensorView g, TensorView beta,
                  ...
                  TensorView split_state, TensorView map_state, TensorView carry,
                  TensorView split_out, TensorView map_state_bf16,
                  int64_t prepare_descriptors, int64_t num_heads, int64_t use_initial_state,
                  double scale, double lower_bound, int64_t num_parts, int64_t cuda_stream) {
  TVM_FFI_ICHECK(cuda_stream >= 0) << "cuda_stream must be a non-negative stream handle";
  TVM_FFI_ICHECK(q.device().device_type == kDLCUDA) << "q must be a CUDA tensor";
  const int32_t device_id = q.device().device_id;
  ffi::CUDADeviceGuard device_guard(device_id);
  ...
  const int64_t slab = VibeCUDASlabRegime(token_count, num_heads);
  if (slab != 0) {
    kda_flash_slab::RunM128SplitFull(q.data_ptr(), k.data_ptr(), v.data_ptr(), g.data_ptr(),
                                     beta.data_ptr(), beta_tma.data_ptr(), A_log.data_ptr(),
                                     dt_bias.data_ptr(), cu_seqlens.data_ptr(),
                                     seq_order.data_ptr(), initial_state.data_ptr(),
                                     out.data_ptr(), final_state.data_ptr(),
                                     descriptor_storage.data_ptr(), token_count, num_seqs,
                                     prepare_descriptors, num_heads, use_initial_state, scale,
                                     lower_bound, beta_tma_rows, beta_tma_dim1, num_parts,
                                     split_state.data_ptr(), map_state.data_ptr(),
                                     carry.data_ptr(), split_out.data_ptr(),
                                     /*lookback_flags_ptr=*/nullptr, map_state_bf16.data_ptr(),
                                     slab, cuda_stream);
  } else {
    kda_flash::RunM128SplitFull(q.data_ptr(), k.data_ptr(), v.data_ptr(), g.data_ptr(),
                                beta.data_ptr(), beta_tma.data_ptr(), A_log.data_ptr(),
                                dt_bias.data_ptr(), cu_seqlens.data_ptr(), seq_order.data_ptr(),
                                initial_state.data_ptr(), out.data_ptr(), final_state.data_ptr(),
                                descriptor_storage.data_ptr(), token_count, num_seqs,
                                prepare_descriptors, num_heads, use_initial_state, scale,
                                lower_bound, beta_tma_rows, beta_tma_dim1, num_parts,
                                split_state.data_ptr(), map_state.data_ptr(), carry.data_ptr(),
                                split_out.data_ptr(), /*lookback_flags_ptr=*/nullptr,
                                map_state_bf16.data_ptr(), slab, cuda_stream);
  }
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(run_persistent_m128, flashinfer::vibecuda_flashkda::RunPersistentM128);
