...
struct HolisticPlanInfo {
  ...
  struct {
    ...
    int64_t barrier_base_offset;    // [num_seqs] base barrier index per sequence (for split-KV barriers)
  ...
  static constexpr uint32_t NUM_DYN_ARGS = 13;  // per-task dynamic scheduler args (added barrier_base)
  static constexpr uint32_t NUM_SHARED_ARGS = 13 + NUM_TASKS + 1 + NUM_TASKS * NUM_DYN_ARGS + 3;  // +3 for dyn_scalars_offset, enable_cuda_graph, flipped_schedule (dup at end)
  ...
  std::vector<int64_t> ToVector() const {
    ...
    for (uint32_t i = 0; i < NUM_TASKS; ++i) {
      ...
      vec.push_back(dyn[i].barrier_base_offset);
    ...
    // flipped_schedule must be last — Python reads plan_info[-1] to auto-select kernel
    vec.push_back(flipped_schedule);
    return vec;
  }
  ...
  void FromVector(const std::vector<int64_t>& vec) {
    ...
    merged_barrier_idx_offset = vec[base + 5];
    num_qo_len_offset = vec[base + 6];
    barrier_vec_offset = vec[base + 7];
    barrier_vec_copy_offset = vec[base + 8];
    barrier_vec_size = vec[base + 9];
    flipped_schedule = vec[base + 10];
    for (uint32_t i = 0; i < NUM_TASKS; ++i) {
      total_num_works[i] = vec[base + 11 + i];
    }
    tile_counter_offset = vec[base + 11 + NUM_TASKS];
    uint32_t dyn_base = base + 11 + NUM_TASKS + 1;
    for (uint32_t i = 0; i < NUM_TASKS; ++i) {
      ...
      dyn[i].barrier_base_offset = vec[dyn_base + i * NUM_DYN_ARGS + 8];
      dyn[i].num_seqs = vec[dyn_base + i * NUM_DYN_ARGS + 9];
      dyn[i].total_tiles = vec[dyn_base + i * NUM_DYN_ARGS + 10];
      dyn[i].len_kv_chunk = vec[dyn_base + i * NUM_DYN_ARGS + 11];
      dyn[i].uniform_num_m_blocks = vec[dyn_base + i * NUM_DYN_ARGS + 12];
    }
    uint32_t extra_base = dyn_base + NUM_TASKS * NUM_DYN_ARGS;
    dyn_scalars_offset = vec[extra_base + 0];
    enable_cuda_graph = vec[extra_base + 1];
  }
};
...
inline cudaError_t TwoStageHolisticPlan(void* float_buffer, size_t float_workspace_size_in_bytes,
  ...
  // Pre-compute kv_len_limit per task (needed for KV splitting check in dynamic scheduler)
  auto f_kv_limit = [](int x) {
  ...
  for (uint32_t task = 0; task < NUM_TASKS; ++task) {
    ...
    precomputed_kv_len_limits[task] = kv_len_limit;
  }

  // Partial output tracking — shared between dynamic and static scheduler paths.
  // Dynamic scheduler's split-KV may add entries before the static scheduler runs.
  int partial_o_nnz = 0;
  std::vector<IdType> merge_indptr, merge_o_indices;
  merge_indptr.push_back(partial_o_nnz);

  // L2 cache budget for head grouping (dynamic scheduler only)
  int l2_cache_size = 0;
  ...
  std::vector<IdType> num_expand_qo_len_vec;
  std::vector<IdType> cluster_len_kv_chunk(NUM_TASKS, 0);

  // Barrier tracking for per-output-row semaphore synchronization
  std::vector<uint32_t> merged_barrier_idx, barrier_vec;
  uint32_t barrier_idx = 0;

  // Single loop over tasks.
  // Dynamic scheduler (LPT) when flipped_schedule.
  // Static scheduler always runs (CG may replay captured flipped=false kernel on any batch).
  for (uint32_t task = 0; task < NUM_TASKS; ++task) {
    int cluster_tile_q = CTA_TILE_Q_SIZES[task] * cluster_size;
    int kv_len_limit = precomputed_kv_len_limits[task];
    cluster_len_kv_chunk[task] = kv_len_limit;
    auto& seqs = idx_qo_kv_len_vec[task];
    int num_seqs = seqs.size();

    if (plan_info.flipped_schedule) {
      // ===== Dynamic scheduler (LPT): per-sequence metadata =====
      std::vector<IdType> dyn_qo_indptr_vec(num_seqs, 0);
      ...
      std::vector<uint32_t> dyn_barrier_base_vec(num_seqs, 0);
      int total_tiles = 0;
      ...
      for (int s = 0; s < num_seqs; ++s) {
        ...
        // Split-KV partial output addressing + barrier tracking: per q_tile
        {
          bool any_qt_splits = false;
          dyn_partial_o_offset_vec[s] = partial_o_nnz;
          dyn_barrier_base_vec[s] = barrier_idx;
          for (int qt = 0; qt < num_m_blocks; ++qt) {
            ...
            if (qt_split) {
              ...
              for (int row = 0; row < row_tile_size; ++row) {
                ...
                // merged_barrier_idx: layout [packed_qo_len, num_kv_heads]
                for (uint32_t kv_head = 0; kv_head < num_kv_heads; ++kv_head) {
                  merged_barrier_idx.push_back(barrier_idx + kv_head);
                }
              }
              // barrier_vec: initial semaphore count = num_kv_chunks for this q_tile
              for (uint32_t kv_head = 0; kv_head < num_kv_heads; ++kv_head) {
                barrier_vec.push_back(qt_chunks);
              }
              barrier_idx += num_kv_heads;
              partial_o_nnz += row_tile_size * qt_chunks;
            }
            // Non-splitting q_tiles write through — no merge_indptr/barrier entries
          }
          if (!any_qt_splits) {
            dyn_partial_o_offset_vec[s] = 0;
            dyn_barrier_base_vec[s] = 0;
          }
        }
      }
      ...
      plan_info.dyn[task].barrier_base_offset =
          int_allocator.aligned_alloc_offset(sizeof(uint32_t) * alloc_seqs, 16, "dyn_barrier_base");
      ...
      if (num_seqs > 0) {
        ...
        CopyToPageLockedBuffer(page_locked_int_buffer, plan_info.dyn[task].barrier_base_offset, dyn_barrier_base_vec);
      }
      ...
      plan_info.tasks[task].q_len_offset = 0;
      plan_info.tasks[task].kv_len_offset = 0;
      plan_info.tasks[task].q_start_offset = 0;
      plan_info.tasks[task].kv_start_offset = 0;
      plan_info.tasks[task].kv_end_offset = 0;
      plan_info.tasks[task].kv_head_idx_offset = 0;
      plan_info.tasks[task].barrier_idx_offset = 0;
    } else {
      ...
      plan_info.dyn[task].barrier_base_offset = 0;
    ...
    for (auto& [i, qo_len, kv_len] : idx_qo_kv_len_vec[task]) {
      ...
      for (int qo_tile_idx = 0; qo_tile_idx < num_qo_tiles; ++qo_tile_idx) {
        ...
        if (split_kv) {
          ...
          // barrier_vec: initial semaphore count = num_kv_tiles for this q_tile
          for (uint32_t kv_head = 0; kv_head < num_kv_heads; ++kv_head) {
    ...
    if (plan_info.flipped_schedule && total_num_works > 0) {
      ...
      {
        std::vector<uint32_t> tmp(barrier_idx_vec.size());
        for (int i = 0; i < total_num_works; ++i) tmp[i] = barrier_idx_vec[perm[i]];
        barrier_idx_vec = std::move(tmp);
      }
    }
    ...
    plan_info.tasks[task].barrier_idx_offset =
        int_allocator.aligned_alloc_offset(sizeof(uint32_t) * max_total_num_works, 16, "barrier_idx");
  ...
  // Barrier arrays for per-output-row semaphore synchronization
  plan_info.merged_barrier_idx_offset = int_allocator.aligned_alloc_offset(
      sizeof(uint32_t) * std::max((size_t)1, merged_barrier_idx.size()), 16, "merged_barrier_idx");
  plan_info.barrier_vec_offset =
      int_allocator.aligned_alloc_offset(sizeof(uint32_t) * std::max((size_t)1, barrier_vec.size()), 16, "barrier_vec");
  plan_info.barrier_vec_copy_offset = int_allocator.aligned_alloc_offset(
      sizeof(uint32_t) * std::max((size_t)1, barrier_vec.size()), 16, "barrier_vec_copy");
  plan_info.barrier_vec_size = barrier_vec.size();
  ...
  plan_info.enable_cuda_graph = enable_cuda_graph;
  std::vector<IdType> dyn_scalars_vec(NUM_TASKS * DYN_SCALARS_PER_TASK);
  for (uint32_t t = 0; t < NUM_TASKS; ++t) {
    dyn_scalars_vec[t * DYN_SCALARS_PER_TASK + 0] = plan_info.dyn[t].num_seqs;
