...
def _run_vibecuda_kda_prefill(
    *,
    ...
    stream_workspace = (
        _get_vibecuda_stream_workspace(q.device)
    ...
    with workspace._lock:
        ...
        if split_parts >= 2:
            split_state, map_state, carry, map_state_bf16 = _vibecuda_split_buffers(
                state, q.device,
                num_sequences * num_heads, split_parts,
            )
            split_out = _vibecuda_split_out_buffer(
                state, q.device, out_buf.numel()
            )
        persistent_buffers = None
        if variant == "persistent":
            persistent_buffers = _vibecuda_persistent_planner_buffers(
        ...
        try:
            if variant == "m64":
                module.run_m64(
                    q, k, v, g, beta, beta_tma, A_log, dt_bias, cu_seqlens_i64,
                    seq_order_i32, initial_state_arg, out_buf, final_state_arg,
                    descriptor_storage, prepare_descriptors, num_heads,
                    int(use_initial_state), int(store_final_state), scale_value,
                    float(lower_bound), stream_ptr,
                )
            elif variant == "persistent":
                task_ids, task_offsets, choice = (
                    _vibecuda_persistent_planner_buffers(
                        state, q.device, num_sequences * num_heads, sm_count
                    )
                )
                module.run_persistent_m128(
                    q, k, v, g, beta, beta_tma, A_log, dt_bias, cu_seqlens_i64,
                    seq_order_i32, task_ids, task_offsets, choice,
                    initial_state_arg, out_buf, final_state_arg,
                    descriptor_storage, prepare_descriptors, num_heads,
                    int(use_initial_state), scale_value, float(lower_bound),
                    sm_count, stream_ptr,
                )
            elif split_parts >= 2:
                module.run_m128_split(
                    q, k, v, g, beta, beta_tma, A_log, dt_bias, cu_seqlens_i64,
                    seq_order_i32, initial_state_arg, out_buf, final_state_arg,
                    descriptor_storage, split_state, map_state, carry,
                    split_out, map_state_bf16, prepare_descriptors, num_heads,
                    int(use_initial_state), scale_value, float(lower_bound),
                    split_parts, stream_ptr,
                )
            else:
                module.run_m128(
                    q, k, v, g, beta, beta_tma, A_log, dt_bias, cu_seqlens_i64,
