...
if __name__ == "__main__":
    ...
    max_decode_tokens = 100 * 4000 # The server and hold a max of bs = 100, avg kv len = 4000
    randgen = random.Random(args.seed)
    
    # Collect selected SMs (10 per run, grouped by unique configurations)
    all_selected_sms = []
    
    for p_len, d_len in tqdm(seq_len_combinations, desc="Profiling sequence length combinations"):
        max_bs = max_decode_tokens // d_len
        batch_sizes = [max_bs, randgen.randint(int(max_bs * 0.2), int(max_bs * 0.7)), randgen.randint(int(max_bs * 0.5), int(max_bs * 0.9))] # simulate below-max server load
        ...
        for causal, batch_size in zip(causal_list, batch_sizes):
            ...
            if df is not None and len(df) > 0:
                ...
                grouped = df.groupby(['prefill_qo_len', 'prefill_kv_len', 'decode_qo_len', 'decode_kv_len'])
                ...
                for i, group_key in enumerate(unique_groups):
                    ...
                    if len(group_queues[group_key]) > 0:
                        ...
                        group_queues[group_key] = group_queues[group_key].iloc[1:].reset_index(drop=True)
                        print(f"Configuration {i}: prefill_qo_len={group_key[0]}, prefill_kv_len={group_key[1]}, decode_qo_len={group_key[2]}, decode_kv_len={group_key[3]}")
                
                # Second pass: if we have fewer than datapoints_per_config, pop more from existing groups
                group_idx = 0
                while len(selected_rows) < datapoints_per_config and len(unique_groups) > 0:
                    group_key = unique_groups[group_idx]
                    if len(group_queues[group_key]) > 0:
                        selected_rows.append(group_queues[group_key].iloc[[0]].copy())
                        group_queues[group_key] = group_queues[group_key].iloc[1:].reset_index(drop=True)
                        print(f"  Filling duplicate from config {group_idx}: prefill_qo_len={group_key[0]}, prefill_kv_len={group_key[1]}, decode_qo_len={group_key[2]}, decode_kv_len={group_key[3]}")
                    group_idx = (group_idx + 1) % len(unique_groups)
                    # Stop if all queues are empty
                    if all(len(q) == 0 for q in group_queues.values()):
                        print(f"  All queues empty, stopping at {len(selected_rows)} SMs")
                        break
                ...
                if selected_rows:
                    ...
                    print(f"Run: Selected {len(selected_rows)} SMs from {len(unique_groups)} unique seqlen configurations")
    ...
    if all_selected_sms:
        ...
        output_name = ("sm_performance_final_flipped" if args.flipped else "sm_performance_final") + "_" + time.strftime("%Y%m%d%H%M%S")
